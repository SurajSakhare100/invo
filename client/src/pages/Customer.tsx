...
import { CUSTOMER_TABS, PAGE_LIMIT, COUNTRIES, SORT_OPTIONS, parseSortValue } from '../data/data';
import type { Customer, CustomerFormData } from '../types';

type TabType = typeof CUSTOMER_TABS[number];
...
export default function Customer() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['customers', activeTab, debouncedSearch, page, filters],
    queryFn: () =>
  ...
  const handleSearchChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setSearchQuery(val);
      setPage(1);
      clearTimeout((window as unknown as { _custSearchTimer: ReturnType<typeof setTimeout> })._custSearchTimer);
      (window as unknown as { _custSearchTimer: ReturnType<typeof setTimeout> })._custSearchTimer = setTimeout(
  ...
  const allSelected =
    customers.length > 0 && customers.every((c) => selected.includes(c._id));
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF9] text-gray-900 font-sans flex">
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-3.5 relative">
            <FilterDropdown
              ...
              prefix={
                <div className="w-7 h-5 rounded bg-emerald-600 text-white text-[9px] font-bold flex items-center justify-center">
                  🌍
                </div>
              }
              options={[
            ...
            >
              <FilterField label="Sort by">
                ...
                >
                  {SORT_OPTIONS.customers.map((opt) => (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          {isError ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-gray-700 font-semibold mb-1">Failed to load customers</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-gray-400 text-[13px] bg-gray-50/50">
                  ...
                  <th className="text-left font-semibold py-3.5 px-3">Customer ID</th>
                  <th className="text-left font-semibold py-3.5 px-3">Name</th>
                  <th className="text-left font-semibold py-3.5 px-3">Contact</th>
                  <th className="text-left font-semibold py-3.5 px-3">Company</th>
                  <th className="text-left font-semibold py-3.5 px-3">Location</th>
                  <th className="text-left font-semibold py-3.5 px-3">Status</th>
                  <th className="text-left font-semibold py-3.5 px-3">Joined</th>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  customers.map((cust) => (
                    <tr
                      key={cust._id}
                      className={`border-b border-gray-50 last:border-b-0 hover:bg-gray-50/60 transition-colors ${selected.includes(cust._id) ? 'bg-green-50/30' : ''
                        }`}
                    >
                      <td className="py-4 pl-5">
                        <input
                          type="checkbox"
                          checked={selected.includes(cust._id)}
                          onChange={() => toggleRow(cust._id)}
                          className="w-4 h-4 rounded border-gray-300 cursor-pointer accent-black"
                        />
                      </td>
                      ...
                      <td className="py-4 px-2 text-gray-400 relative">
                        <button
                          onClick={() =>
                            setOpenMenu(openMenu === cust._id ? null : cust._id)
                          }
      ...
      <CustomerModal
        ...
        onSubmit={async (data) => {
          if (editCustomer) {
            await updateMutation.mutateAsync({ id: editCustomer._id, data });
          }
        }}
      ...
      <DeleteConfirmModal
        ...
        message={`Are you sure you want to delete ${selected.length} selected customers? This action cannot be undone.`}
